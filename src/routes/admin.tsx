...
import type { Pin, Testimonial, Feedback } from "@/lib/db";
...
function AdminPage() {
  ...
  const fetchData = async () => {
    try {
      const [p, t, f] = await Promise.all([getPinsFn(), getTestimonialsFn(), getFeedbackFn()]);
      setPins(p || []);
      setTestimonials(t || []);
      setFeedbacks(f || []);
    } catch (error) {
  ...
  const onPinSubmit = async (values: z.infer<typeof pinSchema>) => {
    ...
    try {
      ...
      await (addPinFn as any)({ data: formData });
  ...
  const onTestimonialSubmit = async (values: z.infer<typeof testimonialSchema>) => {
    ...
    try {
      ...
      await (addTestimonialFn as any)({ data: formData });
  ...
  const executeDelete = async (id: string, type: 'pin' | 'testimonial' | 'feedback') => {
    try {
      if (type === 'pin') await (deletePinFn as any)({ data: id });
      else if (type === 'testimonial') await (deleteTestimonialFn as any)({ data: id });
      else if (type === 'feedback') await (deleteFeedbackFn as any)({ data: id });
    ...
    <div className="min-h-screen bg-[#09090b] font-sans text-zinc-100">
      ...
      <header className="sticky top-0 z-50 w-full border-b border-zinc-800 bg-[#09090b]/80 backdrop-blur-md px-6 h-16 flex items-center justify-between">
        <div className="flex items-center gap-4">
          <img src={logo} alt="NVision" className="h-8 w-auto brightness-110" />
            ...
            <div className="flex items-center">
              <TabsContent value="projects" className="m-0">
                <Dialog open={isAddProjectOpen} onOpenChange={setIsAddProjectOpen}>
                  ...
                  <DialogContent className="bg-zinc-950 border-zinc-800 text-white animate-none duration-0 data-[state=open]:animate-none data-[state=closed]:animate-none">
                    <DialogHeader>
                    ...
                    <tr key={p.id} className="hover:bg-zinc-900/30 transition-colors">
                      ...
                      <td className="px-6 py-4 text-right">
                        <AlertDialog>
                          ...
                          <AlertDialogContent className="bg-zinc-950 border-zinc-800 animate-none duration-0 data-[state=open]:animate-none data-[state=closed]:animate-none">
                            <AlertDialogHeader>
